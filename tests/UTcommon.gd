...
func drag_card(card: Card, target_position: Vector2, interpolation_speed := "fast"):
...
func target_card(source: Card,
		target: Card, interpolation_speed := "fast"):
	var mouse_speed = MOUSE_SPEED[interpolation_speed][0]
	var mouse_yield_wait = MOUSE_SPEED[interpolation_speed][1]
	if source == target:
...
func execute_with_yield(card: Card):
	await card.execute_scripts()


func execute_with_target(card: Card, target: Card):
