...
class TestFilterGtGeLtLe:
	...
	func test_property_filter_gt_ge_lt_le():
		...
		await yield_to(cards[3]._tween, "tween_all_completed", 0.2)
		...
		card.scripts = {"manual": {"hand": [
				...
				"degrees": 270}]}}
		await card.execute_scripts()
		await yield_to(cards[1]._tween, "tween_all_completed", 0.2)
...
class TestAnd:
	...
	func test_and():
		...
		await yield_to(target._tween, "tween_all_completed", 0.2)
	...
	func test_or():
		...
		card.scripts = {"manual": {"hand": [
				...
				"set_faceup": false}]}}
		await card.execute_scripts()
		await yield_to(target._flip_tween, "tween_all_completed", 0.4)
...
class TestPerCounter:
	...
	func test_per_counter():
		...
		await card.execute_scripts()
		cards[1].execute_scripts()
		await yield_to(card._flip_tween, "tween_all_completed", 0.4)
