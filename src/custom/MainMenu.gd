...
#	settings_menu.recover_prebuilts.connect("pressed", self, "_on_PreBuilts_pressed")
	var db_back = $DeckBuilder.find_child("BackButton", true, false)
	if db_back:
		db_back.pressed.connect(_on_DeckBuilder_Back_pressed)
	var cl_back = $CardLibrary.find_child("BackButton", true, false)
	if cl_back:
		cl_back.pressed.connect(_on_CardLibrary_Back_pressed)
...
#			main_position_x = -get_viewport().size.x
		deck_builder, card_library:
			main_position_x = get_viewport().size.x
	var menu_tween: Tween = create_tween()
	menu_tween.set_parallel(true)
	menu_tween.tween_property(main_menu, 'position:x',
			main_position_x, menu_switch_time).from(main_menu.position.x).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
	menu_tween.tween_property(tab, 'position:x',
			0, menu_switch_time).from(tab.position.x).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
...
#			tab_position_x = get_viewport().size.x
	...
	menu_tween.set_parallel(true)
	menu_tween.tween_property(tab, 'position:x',
			tab_position_x, menu_switch_time).from(tab.position.x).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
	menu_tween.tween_property(main_menu, 'position:x',
			0, menu_switch_time).from(main_menu.position.x).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
