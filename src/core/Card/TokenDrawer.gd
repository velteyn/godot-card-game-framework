...
func token_drawer(requested_state := true):
	...
	if not (_tween and _tween.is_running()) and \
			not (owner_card._flip_tween and owner_card._flip_tween.is_running()) and \
			not (owner_card._tween and owner_card._tween.is_running()):
		...
		if requested_state == true and $Drawer/VBoxContainer.get_child_count():
			...
			_tween.tween_property(
					...
					0.3).from(td.position).set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_OUT)
			...
			_tween.tween_property(
					...
					td.position.y),
					0.2).from(td.position).set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_IN)
