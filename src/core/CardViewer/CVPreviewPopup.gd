	...
	if not _placement_initialized:
		...
		if _tween_wait > 10:
			...
			if _tween and _tween.is_valid():
				_tween.kill()
			_tween = create_tween()
			_tween.tween_property(self, "position", new_position, 0.2).from(position).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_IN)
...
func get_preview_placement() -> Vector2:
	...
	var card_offscreen_y = mouse_pos.y\
			+ card_size.y
	var focus_offscreen_y = mouse_pos.y + focus_info.size.y
