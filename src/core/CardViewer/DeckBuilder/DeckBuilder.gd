...
func _on_Save_pressed() -> void:
	...
	if not DirAccess.dir_exists_absolute(CFConst.DECKS_PATH):
		DirAccess.make_dir_recursive_absolute(CFConst.DECKS_PATH)
	var file = FileAccess.open(CFConst.DECKS_PATH + _deck_name.text + '.json', FileAccess.WRITE)
	file.store_string(JSON.new().stringify(deck_dictionary, '\t'))
	file.close()
...
func _set_notice(text: String, colour := Color(0,1,0)) -> void:
	...
	tween.tween_property(_notice, "modulate:a", 0, 2).from(1).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
