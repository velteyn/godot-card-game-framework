...
func set_card_rotation(
	...
	# and a toggle was not requested, we consider we did nothing
		...
		if not state in [CardState.PREVIEW, CardState.DECKBUILDER_GRID]\
				...
				and not (_tween and _tween.is_running()):
...
func get_card_class(): return "Card"
...
func _organize_attachments() -> void:
	...
	if get_parent() == cfc.NMAP.board:
		for card in attachments:
			...
			if not (card._tween and card._tween.is_running()) and \
					card.state in \
...
func _flip_card(to_invisible: Control, to_visible: Control, instant := false):
	...
	# So we skip that.
		...
		if _flip_tween and _flip_tween.is_valid(): _flip_tween.kill()
		_flip_tween = create_tween().set_parallel(true)
		_flip_tween.tween_property(to_invisible, 'scale',
				Vector2(0,1), 0.4).from(to_invisible.scale).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		_flip_tween.tween_property(to_invisible, 'position',
				Vector2(to_invisible.size.x/2,0), 0.4).from(to_invisible.position).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		_flip_tween.tween_property(highlight, 'scale',
				Vector2(0,1), 0.4).from(highlight.scale).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		_flip_tween.tween_property(highlight, 'position',
				Vector2((highlight.size.x-3)/2,0), 0.4).from(highlight.position).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		await _flip_tween.finished
		to_visible.visible = true
		to_invisible.visible = false
		if _flip_tween and _flip_tween.is_valid(): _flip_tween.kill()
		_flip_tween = create_tween().set_parallel(true)
		_flip_tween.tween_property(to_visible, 'scale',
				Vector2(1,1), 0.4).from(to_visible.scale).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		_flip_tween.tween_property(to_visible, 'position',
				Vector2(0,0), 0.4).from(to_visible.position).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		_flip_tween.tween_property(highlight, 'scale',
				Vector2(1,1), 0.4).from(highlight.scale).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		_flip_tween.tween_property(highlight, 'position',
				Vector2(-3,-3), 0.4).from(highlight.position).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
...
func _add_tween_rotation(
	...
	if not _tween or not _tween.is_valid():
		_tween = create_tween()
	var tweener = _tween.tween_property($Control, 'rotation_degrees', target_rotation, runtime)
	if tweener:
...
#			print(global_position + _control.rect_size)
			...
			if not (_tween and _tween.is_running()) and \
