...
37. **Pile Control labels/buttons hidden behind cards — RESOLVED**: In Godot 3, Control children of CanvasItem parents rendered in a **separate pass** always on top of non-Control children. Godot 4 merged all children into a single `z_index` sort. Fix applied in three parts:
   ...
   - **Mouse routing** (`input_pickable`): In Godot 4, `Area2D` has two separate input properties. `monitorable = false` (already set) only prevents *other* areas from detecting the card. `input_pickable` (defaulting to `true`) controls whether *this* area absorbs mouse hover/click events — cards in piles had this still `true`, silently consuming all mouse hover before the Pile's Area2D could see it. Fix: added `input_pickable = value` to `CardTemplate.set_control_mouse_filters()`, so cards entering piles also set `input_pickable = false`, letting events fall through to the Pile. Also connected the Pile's own `Area2D.mouse_entered`/`mouse_exited` as a second path in `CardContainer._init_signal()`.
   - **Card count label**: Added a sync in `Pile._process()` that keeps `card_count_label.text` up to date every frame. This handles the case where cards are added via raw `add_child` (e.g. `load_test_cards`), which bypasses `_pile_add_card` where the label was previously only updated.
