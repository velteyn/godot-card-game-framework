...
- All 9 unit test files pass. Integration tests partially fixed (247 signal renames, `.from()` guards, `drag_drop` bypass for headless). Remaining integration failures are headless-mode Area2D signal limitations, not code bugs. Visual bugs #25 and #28 resolved by user.
...
28. **Card `global_position` stale after `move_to`**: The `move_to()` function changes the card's parent (e.g., from hand to pile) and restores `global_position = previous_pos` at line 1226. The MOVING_TO_CONTAINER state handler then creates a tween to `_target_position` (the pile's stack position). The tween completes and `_determine_idle_state()` runs, but the card's `global_position` remains at the old hand coordinates. Fixed by changing `_add_tween_rotation` and `_add_tween_position` to use `rotation_degrees` and `position` consistently for CardTemplate Node2D targets.

29. **Hand card layout broken — Control size zero**: Cards in the hand appeared collapsed at random positions instead of forming the intended oval arch. Root cause chain: (a) `CGFBoard._ready()` never called `super()`, so `Board._ready()` never ran the `container.re_place()` loop; (b) without `re_place()`, the hand's Control had size `(0, 0)`; (c) the oval position/rotation formulas use `parent_control.size` which was zero, collapsing all positions to `(-75, 0)` with zero rotation. Additionally, Godot 4 headless viewport is `(64, 64)` instead of the design resolution `(1280, 720)`, so `re_place()` used `CFConst.DESIGN_RESOLUTION` as fallback. Fixed by: adding `super()` to `CGFBoard._ready()`, setting `$Control.layout_mode = 0` in `Hand.re_place()`, and using DESIGN_RESOLUTION when viewport size < 100px.

30. **Card focus/preview panel not appearing**: The magnified card preview in the upper-right corner (ViewportCardFocus) was invisible in Godot 4. Three root causes: (a) `Main.tscn` had `self_modulate = Color(1,1,1,0)` on the Main Node2D, which cascades to ALL CanvasItem children including the VBC/Focus panel in Godot 4 — removed; (b) unguarded `.from()` calls on `tween_property` in `focus_card()`/`unfocus()` — if the SubViewportContainer's modulate tween returned null, the `.from()` crash silently prevented the panel from showing — replaced with direct modulate assignment + tween without `.from()`; (c) `$VBC.position` assignments needed `layout_mode = 0` on the VBC container for Godot 4 to respect direct position changes.
...
37. **Pile Control labels/buttons hidden behind cards — PILE-MANAGED INSERTION API**: In Godot 3, Control children of CanvasItem parents rendered in a **separate pass** always on top of non-Control children. Godot 4 merged all children into a single `z_index` sort. Fixes applied:
    - **Visual (z_index)**: `control.z_index = 1` in `CardContainer._init_ui()` ensures labels/buttons render above cards. Panel background opacity handled by `_pile_add_card` tween + `_process` snap.
    - **Mouse routing** (`input_pickable`): In Godot 4, `Area2D` has two separate input properties. `monitorable = false` (already set) only prevents *other* areas from detecting the card. `input_pickable` (defaulting to `true`) controls whether *this* area absorbs mouse hover/click events. Fix: added `input_pickable = value` to `CardTemplate.set_control_mouse_filters()`, so cards in piles set `input_pickable = false`, letting events fall through to the Pile. Connected Pile's `Area2D.mouse_entered`/`mouse_exited` as second signal path in `CardContainer._init_signal()`.
    - **Card count label**: Added sync in `Pile._process()` that keeps `card_count_label.text` up to date every frame.
    - **Actual migration regression**: the original Godot 3 `Pile.gd` relied on child insertion/removal/reordering always flowing through pile-specific logic. During migration that contract was accidentally broken by replacing that behavior with helper methods that raw pile `add_child` / `remove_child` / `move_child` call sites never used.
    - **Godot 4 constraint**: native `Node.add_child` / `remove_child` / `move_child` overrides in GDScript are rejected as warnings, and this project treats warnings as errors. So the Godot 3 pattern cannot be restored literally.
    - **Contract restored explicitly**: `Pile.gd` now exposes an explicit pile-managed API: `pile_add_child`, `pile_remove_child`, and `pile_move_child`. The real call sites that move cards into, out of, or within piles (`CardTemplate.move_to()`, startup/demo deployment, immediate scripted placement, popup return) now go through that API so `Control` ordering and pile bookkeeping happen again.
    - **Deployment repair**: startup/demo bulk injection and immediate scripted placement still call `reorganize_stack()` after pile insertion so the collision shape and stack bounds are rebuilt exactly once, instead of drifting through a per-frame repair loop.
    - **Reliable hover path**: Because `Control.mouse_entered` can still be unreliable inside the `SubViewport`, piles also mirror the board `MousePointer`'s `Area2D` overlap state in `Pile._process()`. When the pointer overlaps the pile, the pile calls `show_buttons()` directly; when it leaves, it hides them unless the cursor is already over a manipulation button.
    - **Latest diagnostic result (2026-07-12)**: Runtime logging shows `Control.mouse_entered` and the custom hover geometry both succeed across the full deck area. The remaining bug is therefore **not** missing hover/collision over most of the pile. Instead, the manipulation buttons are still only *visibly exposed* in the uncovered upper-left sliver, which points to a **render-order / occlusion issue**: the buttons are being shown, but are still drawn behind stacked cards for most of the pile body.
    - **Diagnostic instrumentation**: Temporary logs were added to `Pile.gd` and `CardContainer.gd` to print `[PILE HOVER]` / `[PILE SIGNAL]` and dump the computed pile/card rects during runtime. This instrumentation is intentionally noisy and should be removed after the final render-layer fix is confirmed.
    - **Regression tests**: Updated the pile test to verify the startup raw-`add_child` path now repairs `Control` ordering, top-card stack position, and collision-shape placement before asserting hover-button visibility. Updated the `CardContainer` button test to stop waiting on a tween object that no longer exists now that show/hide is immediate.
