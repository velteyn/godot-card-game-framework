...
37. **Pile Control labels/buttons hidden behind cards — PARTIALLY RESOLVED**: In Godot 3, Control children of CanvasItem parents rendered in a **separate pass** always on top of non-Control children. Godot 4 merged all children into a single `z_index` sort. Fixes applied:
    ...
    - **Remaining issue**: Hover detection (`mouse_entered`) on the pile Control still doesn't fire when cards overlap the pile area. When cards are manually moved away (deck emptied), the Control receives hover and buttons appear correctly. The card's `input_pickable = false` + `monitorable = false` should make cards fully transparent to input, yet something still blocks the event. The board resides inside a `SubViewport` which adds input dispatch complexity — the SubViewport may route events through the physics layer (Area2D) before the GUI layer (Control), and card collision shapes may still intercept despite disabled flags. Further debugging needed in the editor's remote scene tree to inspect runtime `input_pickable`, `monitorable`, `mouse_filter`, and `z_index` values on both the Pile Control and its card siblings.
    - **Related**: `card_count_label.text` is only updated via `_pile_add_card`/`_pile_remove_card`, which are not called during initial `load_test_cards` (uses raw `add_child`). Card count shows "0" until a reshuffle triggers the label update. Sync added in `_process` as workaround.
